...
public class HodHistoryModel : PageModel
{
    private readonly HistoryListService _history;
    private readonly AdminService _admin;
    private readonly RecordDeleteService _delete;

    public HodHistoryModel(HistoryListService history, AdminService admin, RecordDeleteService delete)
    {
        _history = history;
        _admin = admin;
        _delete = delete;
    ...
    public List<HodHistoryRow> Rows { get; set; } = [];
    ...
    public async Task OnGetAsync(string? from, string? to, string? area, string? q, string? deleted)
    {
        ...
        Rows = await _history.LoadHodAuditsAsync(new HistoryFilters(from, to, area, q));
    }

    public async Task<IActionResult> OnPostDeleteAsync(
        int id, bool isLegacy, string? from, string? to, string? area, string? q)
    {
        if (!_admin.IsAdmin())
            return RedirectToPage(new { from, to, area, q });

        var ok = await _delete.DeleteAsync("hod", id, !isLegacy);
